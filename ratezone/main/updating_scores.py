...
def update_scores(prof_id):
    element = Employee.objects.get(employee=prof_id)
    ...
    count_exams_difficulty = UserFacultyRev.objects.filter(employee_id=element).aggregate(Count('difficulty_rating'))
